...
if __name__ == '__main__':
    ...
    if sample_class == 'cifar10':
        ...
        batch_size = 64
        train_data = DataLoader(dataset=shuffled_data_1, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=True, num_workers=8)
        ...
        train_data_1 = DataLoader(dataset=shuffled_data_2, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=True, num_workers=8)
        ...
        # 图像预处理的转换操作，对图像进行标准化处理，让图像能够适应模型的输入要求
        train_dir=os.path.join('/root/autodl-tmp/imagenet100picture/train_csv', 'train_data1.txt')
        training_data_1 = MyDataset(txt_dir=train_dir,transform=transform)
        ...
        batch_size = 64
        train_data = DataLoader(dataset=training_data, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=True, num_workers=8)
        test_data = DataLoader(dataset=testing_data, batch_size=batch_size, shuffle=True, drop_last=False, pin_memory=True, num_workers=8)
        train_data_1 = DataLoader(dataset=training_data_1, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=True, num_workers=8)
    ...
    if model_class == 'resnet18':
        ...
        model = vgg_model
        # model.load_state_dict(torch.load('/root/autodl-tmp/model/imagenet100_vggclean.pth'))
        ...
        model = densenet
        model.load_state_dict(torch.load('/root/autodl-tmp/model/new_animals_densenet_clean.pth'))
        ...
        model = resnet50
        model.load_state_dict(torch.load('/root/autodl-tmp/model/new_imagenet_resnet50_clean.pth'))
        ...
        model.load_state_dict(torch.load('/root/autodl-tmp/model/new_imagenet_vit_clean.pth'))
    else:
...
# #     tgtoptimizer.load_state_dict(torch.load('/root/model/tgtoptimizer_full_target.pth'))
    ...
    if sample_class == 'cifar10':
        ...
        tgtoptimizer = optim.Adam(tgtmodel.parameters(), lr=0.0001)
        # tgtmodel.load_state_dict(torch.load('/root/autodl-tmp/model/tgtmodel_full_target_changefeature.pth'))
        # tgtoptimizer.load_state_dict(torch.load('/root/model/tgtoptimizer_res_singlet_80.pth'))
    else:
        raise ValueError(f"Unsupported sample class: {sample_class}")
    ...
    dwt = pytorch_wavelets.DWTForward(J=2, wave='haar', mode='zero').to(device)
    ...
    yading2 = cv2.imdecode(np.fromfile('/root/dog.png', dtype=np.uint8), 1)  # 1：彩色；2：灰度
    yading2 = cv2.cvtColor(yading2, cv2.COLOR_BGR2RGB)  # 转化为RGB，也可以用img = img[:, :, (2, 1, 0)]
    yading_tensor_1 = transform(yading2).unsqueeze(0).to(device)
    
    Yl, Yh = dwt(yading_tensor)
    Yl_all, Yh_all = dwt(yading_tensor_1)
    x_train_1 = idwt((Yl, Yh_all))
            
    atkdata_np = denormalize_and_convert_to_uint8(x_train_1[0].clone().detach().cpu(), mean, std)
    atkdata_np_path = f'/root/Pytorch-UNet-master/picture/atkdata_np_dwt.png'
    plt.imsave(atkdata_np_path, atkdata_np)
    cha = atkdata_np-yading1
    atkdata_np_path = f'/root/Pytorch-UNet-master/picture/cha_dwt.png'
    plt.imsave(atkdata_np_path, cha)
    print('dot111')
    ...
    for epoch in range(1, 1001):
        ...
        for batch_idx, (x_train, y_train) in pbar:
            ...
            while True:
                try:
                    x_train_all, y_train_all = next(train_data_all_iter)  # 读取一批数据
                    break
                except StopIteration:
                    # 如果到达 train_data_all 的末尾，则重新创建迭代器
                    train_data_all_iter = iter(train_data_1)
        ...
        with torch.no_grad():
            ...
            if sample_class == 'cifar10':
                ...
                torch.save(tgtmodel.state_dict(), f'/root/autodl-tmp/model/tgtmodel_full_target_changefeature_vgg.pth')
